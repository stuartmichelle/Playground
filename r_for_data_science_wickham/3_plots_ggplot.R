...
# coord_quickmap fixes aspect ratio for maps
...
# Exercises ###################
# Turn a stacked bar chart into a pie chart using coord_polar(), I think this is the x = 1 that makes it pie instead of rings
pie <- ggplot(diamonds, aes(x = factor(1), fill = factor(cut))) +
  geom_bar(width = 1)
pie + coord_polar(theta = "y")

# what does labs() do? modifys axis, legend and plot labels
# what is the difference between coord_map() and coord_quickmap()?
nz <- map_data("nz")
# Prepare a map of NZ
...
# what does the plot below tell you about the relationship between city and hwy mpg?
# why is coord fixed important? It makes sure that 1 on the x axis is equal to 1 on the y axis
# what does geom_abline do? Draws a line using the slope and intercept of the data
