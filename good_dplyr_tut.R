...
flightsSpeed <- flightsDistance  flightsAirTime
flights[, c(Distance, AirTime, Speed)]

# dplyr approach (prints the new variable but does not store it)
flights %%
select(Distance, AirTime) %%
mutate(Speed  DistanceAirTime)

# store the new variable
flights <- flights %% mutate(Speed  DistanceAirTime)
...
head(with(flights, tapply(ArrDelay, , , na.rm)))
head(aggregate(ArrDelay  , flights, ))

# dplyr approach: create a table grouped by Dest, and then summarise each group by taking the mean of ArrDelay
flights %%
group_by() %%
summarise(avg_delay  mean(ArrDelay, na.rm))
...
summarise_each(funs(), Cancelled, Diverted)

# for each carrier, calculate the minimum and maximum arrival and departure delays
flights %%
group_by(UniqueCarrier) %%
summarise_each(funs(min(., na.rm), max(., na.rm)), matches(Delay))
