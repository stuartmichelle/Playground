...
# ------------------------------------------
idcsv <- read.csv(file = "~/Desktop/Katrina_code/09152016_seq_parent-2.csv", stringsAsFactors = F)
...
labor <- src_mysql(dbname = "Laboratory", host = "amphiprion.deenr.rutgers.edu", user = "katrinac", password = "yetistomp", port = 3306, create = F)
...
idcsv <- merge(idcsv, lab1, by.x = "First.candidate.ID", by.y = "First.ligation_id", all.x = T)
...
idcsv <- merge(idcsv, lab2, by.x = "Second.candidate.ID", by.y = "Second.ligation_id", all.x = T)
...
#looks like this is calculating dists wrong for some reason. See id_latlongdist code for trouble shooting
...
write.csv(idcsv, file="id_alldists.csv", sep = "", row.names = F)
